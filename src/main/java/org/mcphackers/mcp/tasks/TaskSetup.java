...
public class TaskSetup extends Task {
    ...
    public void setupMC() throws IOException, URISyntaxException {
        ...
        for (String ver : verList) {
            ...
            tableList[row_index].add(new Ansi().fgBrightYellow().a(" - ").fgDefault().fgCyan().a(String.format("%-12s", ver)).fgDefault().toString());
            index++;
        }
        for (int i = 0; i < tableList.length; i++)
        {
        ...
        while (!versions.contains(chosenVersion)) {
            MCP.logger.print(new Ansi().a("Select version: ").fgGreen());
        ...
        if (!chosenVersion.equals("custom")) {
            ...
            if(json.getJSONObject("client").has(chosenVersion)) {

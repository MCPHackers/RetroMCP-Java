...
public class Decompile implements IBytecodeProvider, IResultSaver {
	...
	public static boolean noPatch = false;
	public int side = 0;
	public DecompileLog log;
	...
	public Decompile(int side) {
		this.side = side;
		this.log = new DecompileLog();
	}
	...
	public void start()
	{
	    ...
	    File destination = new File(MCP.cd + File.separator + "jars"+ File.separator +"src" + File.separator);
	    if (!destination.isDirectory()) {
	    	if(!destination.mkdirs())
	    	{
	    		MCP.logger.println("ERROR: Could not create '" + destination + "'");
		      	return;
	    	}
	    }
	    String source = MCP.cd + File.separator + "jars" + File.separator + (side == 1 ? "minecraft_server.jar" : "minecraft.jar");
        addPath(lstSources, source);

	    if (lstSources.isEmpty()) {
	      MCP.logger.println("ERROR: No sources found");
	      return;
	    }
	    ...
	    private String getAbsolutePath(String path) {
	        return new File(MCP.cd + File.separator + "jars" + File.separator + "src" + File.separator).getAbsolutePath();
	      }
